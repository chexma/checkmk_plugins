#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Contract End Date Monitoring - Active Check Ruleset
# Copyright (C) 2025
...
def _parameter_form():
    return Dictionary(
        ...
        help_text=Help(
            "This active check monitors hardware support contracts by their end date. "
            "The check runs on the CheckMK server and does not require an agent on the target host. "
            "It will alert you when a contract is approaching its expiration based on the configured thresholds."
        ),
        elements={
            "contract_name": DictElement(
                parameter_form=String(
                    title=Title("Contract Name"),
                    help_text=Help("Name of the contract. This will be used as part of the service name."),
                ),
                required=True,
            ),
            "contract_id": DictElement(
                parameter_form=String(
                    title=Title("Contract ID"),
                    help_text=Help(
                        "Optional contract ID or reference number. "
                        "This will be displayed in the service output."
                    ),
                ),
                required=False,
            ),
            "start_date": DictElement(
                parameter_form=String(
                    title=Title("Contract Start Date"),
                    help_text=Help(
                        "Optional contract start date. Use the same date format as selected below. "
                        "This will be displayed in the service output."
                    ),
                ),
                required=False,
            ...
            "date_format": DictElement(
                parameter_form=SingleChoice(
                    ...
                    help_text=Help("Select the format for the dates you entered above."),
                    elements=[
                        SingleChoiceElement(
                            name="iso8601",
...
rule_spec_active_check_contract_enddate = ActiveCheck(
